...
int wait_for_wpa_s_ready(void)
{
	...
	if (wpa_s) {
		LOG_INF("WPA Supplicant is already ready: %s", DEFAULT_WIFI_IFACE);
		return 0;
	}

	ret = k_sem_take(&wpa_supp_ready_sem, K_MSEC(CONFIG_WPAS_READY_TIMEOUT_MS));
	if (ret == -EAGAIN) {
		LOG_INF("Timeout waiting for WPA Supplicant to be ready (%d ms)", CONFIG_WPAS_READY_TIMEOUT_MS);
		return -1;
	}

	wpa_s = z_wpas_get_handle_by_ifname(DEFAULT_WIFI_IFACE);
	if (!wpa_s) {
		LOG_INF("WPA Supplicant ready event received, but no handle found for %s", DEFAULT_WIFI_IFACE);
		return -1;
	}

	/* Belts and braces: Check for ctrl_iface initialization */
	if (wpa_s->ctrl_iface->sock_pair[0] < 0) {
		LOG_INF("WPA Supplicant ready event received, but no control interface socket found for %s", DEFAULT_WIFI_IFACE);
		return -1;
	}

	LOG_INF("WPA Supplicant is ready: %s", DEFAULT_WIFI_IFACE);
